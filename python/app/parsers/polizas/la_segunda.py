...
def _parse_vehiculos(text_paginas: list[str], subtipo: Optional[str], warnings: list[str]) -> list[dict[str, Any]]:
    ...
    Las filas en el texto aparecen así (auto):
    ...
    for text in text_paginas:
        for linea in text.split('\n'):
            ...
            # Número de orden: dígitos al comienzo de la línea.
            m_orden = re.match(r'^\s*(\d+)\s+', linea)
            if m_orden:
                datos['numero_orden_aseguradora'] = m_orden.group(1)

            # Año: 4 dígitos entre 1990-2099 que estén DESPUÉS de la patente.
            tras_patente = linea[m_pat.end():]
            m_anio = re.search(r'\b(19\d{2}|20\d{2})\b', tras_patente)
            if m_anio:
            ...
            m_loc = re.search(r'\b(CORRIENTES|SAN CAYETANO|TUCUMAN|RESISTENCIA|BUENOS AIRES)\b',
                              antes_monto)
            if m_loc:
